...
3. **派生文件勿手改**：`overview.json` / `llms*.txt` / `sitemap.xml` 由 `scripts/` 生成；改数据后跑 `npm run gen`。
...
- **自定义挂载模块**：`demos`（`mountDemos`）、`bigfive`（`mountBigFive`）、`selfrating`（`mountSelfRating`）。不走 ListModule，在 `src/core/app.ts` 的 `boot()` 直接调用，并在 `CUSTOM_MODULES` 集合与 `index.html` 的 host 节点（`#demosRoot` / `#bigfiveRoot` / `#selfratingRoot`）登记。
